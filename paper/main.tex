...
\node[guest, right=14mm of att.north east, anchor=north west] (vq)
  {\textbf{Opfer 1} -- QEMU 301\\ Emulation \emph{(nur Fallback)}};
...
\node[guest, below=2.5mm of vl, fill=green!9, draw=green!45!black] (vk)
  {\textbf{Opfer 3} -- KVM 303\\ HW-Virt.\ \emph{(PoC-Hauptopfer)}};
...
\end{tikzpicture}
\caption{Schematischer Aufbau der Testumgebung. Der Control-Node orchestriert die vier auf denselben physischen P-Core (CPU 4,5) gepinnten Gäste über SSH und misst nicht selbst. Der Angreifer (LXC 300) sättigt den geteilten Last Level Cache; die drei Opfer (QEMU/LXC/KVM) werden sequenziell vermessen, wobei nur das KVM-Opfer als PoC-Hauptopfer dient. Rote Pfeile markieren die Ressourcen-Contention über LLC und I/O.}
\label{fig:setup}
\end{figure}
