...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{Titel-Diskrepanz:} Der vom LLM generierte Titel (,,\textit{Trade-offs der
          ...
          der sachlichen Richtigkeit und der \textbf{Inhaltlichen Validität} (Konstruktvalidität),
          da der tatsächliche Untersuchungsgegenstand im Titel falsch repräsentiert wird. Der
          präzisere Originaltitel des Exposés muss wiederhergestellt werden.
    \item \textbf{Sprachlicher Stil:} Die Formulierung wirkt stellenweise unnatürlich, gezwungen
          intellektuell und künstlich (KI-typische Phrasierung), was den Lesefluss stört. Dies
          widerspricht dem wissenschaftlichen Standard der \textbf{Präzision und Klarheit} (Ökonomie
          des Ausdrucks). Ein wissenschaftlicher Text darf nicht durch dekorative Rhetorik oder
          syntaktische Komplexität Verständlichkeit einbüßen; die Sprache muss natürlicher und
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{Überfokussierung auf Vorab-Literatur:} Der Text baut seine Argumentation
          ...
          ,,Die Systematisierung durch Ge et~al.\ldots''). Dies stellt einen \textbf{strukturellen
          ...
          werden, wobei Quellen lediglich als komprimierter Nachweis in Klammern dienen. Zudem simuliert
          die namentliche Fokussierung eine abgeschlossene Literatursynthese, die zum jetzigen
          Projektzeitpunkt real noch nicht stattgefunden hat, was dem Prinzip der \textbf{Forschungstransparenz}
          widerspricht (Vermeidung von Scheinfundierung).
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{Technologische Überfokussierung:} Die Einleitung verliert sich in einer zu
          ...
          (wie \textsc{Prime+Probe}) verletzt das geforderte \textbf{Abstraktionsniveau} einer Einleitung.
          Eine Einleitung hat die Aufgabe, das Problemfeld konzeptionell und holistisch abzugrenzen; die
          technologische Mikroebene greift der späteren Detailanalyse unzulässig vor. Die Argumentation
          muss sich strikter an die übergeordneten Konzepte der logischen Isolation halten.
    \item \textbf{Formale Artefakte:} Das Dokument enthält strukturelle Überreste des verwendeten
          ...
          aus dem Prompt-Gerüst verletzt den \textbf{formalen Publikationsstandard} (IEEE-Format).
    ...
    \item \textbf{Fehlende Abgrenzung zur Basisaufgabe:} Die Fallback-Strategie wird erwähnt,
          ...
          \textbf{Forschungslogik und methodische Kohärenz}. Es muss wissenschaftlich sauber hergeleitet
          werden, dass der komparative Leistungsvergleich der Virtualisierungslösungen (Basisaufgabe) das
          fundamentale Messbett bildet, auf dessen methodischen Erkenntnissen der spezifische
          Noisy-Neighbor-Angriff erst evaluiert werden kann. Der Fallback darf nicht wie ein isolierter
          Fremdkörper wirken, sondern muss als logische Reduktion des Forschungsdesigns ex ante begründet sein.
  \end{itemize}
\end{itemize}
