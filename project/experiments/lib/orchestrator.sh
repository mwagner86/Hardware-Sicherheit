...
# Führt einen Opfer-Messlauf aus -> Ergebniszeile auf stdout. Args: <user> <host>
victim_run() { rssh "$1" "$2" "${RENV} ${REMOTE_DIR}/victim_benchmark.sh 2>/dev/null"; }

# Sammelt REPEATS Messläufe in eine Roh-CSV.
...
collect_phase() {
    ...
    for i in $(seq 1 "${REPEATS}"); do
        ...
        [[ -n "${cpu}" && -n "${iops}" ]] || die "[${label}] Lauf ${i}: nicht parsebar: '${line}'"
...
# Startet die Angreifer-Störlast für ein Zeitbudget passend zu N Messläufen.
# Args: <user> <host>
attacker_start() {
    local budget=$(( (SYSBENCH_TIME * 2 + FIO_RUNTIME + 30) * REPEATS ))
    rssh "$1" "$2" "${RENV} ${REMOTE_DIR}/attacker_load.sh start ${budget}"
    sleep 5   # Anlaufzeit der Cache-/I/O-Sättigung
}

# Stoppt die Angreifer-Störlast (idempotent). Args: <user> <host>
attacker_stop() {
    rssh "$1" "$2" "${RENV} ${REMOTE_DIR}/attacker_load.sh stop" >/dev/null 2>&1 || true
}
